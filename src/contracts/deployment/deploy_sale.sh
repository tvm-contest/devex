...
function get_address {
    echo $(cat sale/$1.log | grep "Raw address:" | cut -d ' ' -f 3)
}

function genaddr {
    $tos genaddr ../$1.tvc ../$1.abi.json --genkey sale/$1.keys.json > sale/$1.log
}
...
$tos --url $NETWORK deploy ../$ROOT_NAME.tvc sale_params.json \
    --sign sale/$ROOT_NAME.keys.json \
