...
export class CollectionListService {
  ...
  async getCollectionList() : Promise<CollectionInfo[]> {
    ...
    let collectionDirList = fs.readdirSync(globals.TEMP_ROOT, {withFileTypes: true}).filter((fileOrDir) => {
  ...
  private async getCollectionAccount(tempCollectionDir : string) : Promise<Account> {
    let abi = await JSON.parse(fs.readFileSync(path.join(globals.TEMP_ROOT, tempCollectionDir, 'NftRoot.abi.json')).toString());
    let tvc = fs.readFileSync(path.join(globals.TEMP_ROOT, tempCollectionDir, 'NftRoot.tvc'), {encoding: 'base64'});
