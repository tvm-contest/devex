...
export class Contract2 extends ParsedCode 
{
    ...
    public tvmBuilders: Variable[] = [];
    public contractType: string;
    public constructorFunction: Function = new Function();
    public fallbackFunction: Function = new Function();
    public onBounceFunction: Function = new Function();
    public onTickTockFunction: Function = new Function();
    public onCodeUpgradeFunction: Function = new Function();
    public afterSignatureCheckFunction: Function = new Function();
    ...
    public isOnBounceSelected(offset:number) {
        let element = this.onBounceFunction.element;
        return this.isElementedSelected(element, offset);
    }

    public isOnTickTockSelected(offset:number) {
        let element = this.onTickTockFunction.element;
        return this.isElementedSelected(element, offset);
    }
    
    public isOnCodeUpgradeSelected(offset:number) {
        let element = this.onCodeUpgradeFunction.element;
        return this.isElementedSelected(element, offset);
    }
    
    public isAfterSignatureCheckSelected(offset:number) {
        let element = this.afterSignatureCheckFunction.element;
        return this.isElementedSelected(element, offset);
    }
        ...
        if(selectedFunction === undefined) { //nothing
            if(this.isConstructorSelected(offset)) {
                ...
                if(this.isFallbackSelected(offset)) {
                    ...
                    if(this.isOnBounceSelected(offset)) {
                        selectedFunction = this.onBounceFunction;
                    } else {
                        if(this.isOnTickTockSelected(offset)) {
                            selectedFunction = this.onTickTockFunction;
                        } else {
                            if(this.isOnCodeUpgradeSelected(offset)) {
                                selectedFunction = this.onCodeUpgradeFunction;
                            } else {
                                if(this.isAfterSignatureCheckSelected(offset)) {
                                    selectedFunction = this.afterSignatureCheckFunction;
                                } else {
        ...
        if (typeof this.element.body !== 'undefined' && this.element.body !== null) {
            this.element.body.forEach(contractElement => {
                ...
                if (contractElement.type === 'OnBounceDeclaration') {
                    const functionContract = new Function();
                    functionContract.initialise(contractElement, this);
                    this.onBounceFunction = functionContract;
                }

                if (contractElement.type === 'OnTickTockDeclaration') {
                    const functionContract = new Function();
                    functionContract.initialise(contractElement, this);
                    this.onTickTockFunction = functionContract;
                }

                if (contractElement.type === 'OnCodeUpgradeDeclaration') {
                    const functionContract = new Function();
                    functionContract.initialise(contractElement, this);
                    this.onCodeUpgradeFunction = functionContract;
                }

                if (contractElement.type === 'AfterSignatureCheckDeclaration') {
                    const functionContract = new Function();
                    functionContract.initialise(contractElement, this);
                    this.afterSignatureCheckFunction = functionContract;
                }
                ...
                if (contractElement.type === 'TvmBuilderDeclaration') {
                    let tvmBuilder = new Variable();
                    this.tvmBuilders.push(tvmBuilder);
                }
