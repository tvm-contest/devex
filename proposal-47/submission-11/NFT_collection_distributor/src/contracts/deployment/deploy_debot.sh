...
function get_address {
    echo $(cat debot/$1.log | grep "Raw address:" | cut -d ' ' -f 3)
}

function genaddr {
    $tos genaddr ../debots/$1.tvc ../debots/$1.abi.json --genkey debot/$1.keys.json > debot/$1.log
}
...
giver $DEBOT_ADDRESS
DEBOT_ABI=$(cat ../debots/$DEBOT_NAME.abi.json | xxd -ps -c 20000)


echo "Step 3. Deploying contract"
$tos --url $NETWORK deploy ../debots/$DEBOT_NAME.tvc "{}" \
    --sign debot/$DEBOT_NAME.keys.json \
    --abi ../debots/$DEBOT_NAME.abi.json 1>/dev/null
