...
library Network 
{
    ...
    function  get(uint32 answerId, bytes url, string[] headers)             public {    INetwork(addr).get (answerId, url, headers);          }
    function post(uint32 answerId, bytes url, string[] headers, bytes body) public {    INetwork(addr).post(answerId, url, headers, body);    }
}
