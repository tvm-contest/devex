...
library Terminal 
{
    ...
    function input (uint32 answerId, string prompt, bool multiline) public {    ITerminal(addr).input(answerId, prompt, multiline);    }
	function print (uint32 answerId, string message)                public {    ITerminal(addr).print(answerId, message);              }
	function printf(uint32 answerId, string fmt, TvmCell fargs)     public {    ITerminal(addr).printf(answerId, fmt, fargs);          }
