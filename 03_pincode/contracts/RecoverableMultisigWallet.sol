...
contract MultisigWallet is IAccept, RecoverablePubkey {
    ...
    */
    ...
    function _generateId() inline private returns (uint64) {
      m_idCount++ ;
      return (uint64(now) << 32) | m_idCount;
    }
    ...
    function confirmTransaction(uint64 transactionId) public {
        ...
        _removeExpiredTransactions();
        optional(Transaction)  opt_txn = m_transactions.fetch(transactionId);
        require( opt_txn.hasValue() , 102);
        Transaction txn = opt_txn.get() ;
    ...
    function _removeExpiredTransactions() inline private {
        ...
        optional(uint64, Transaction) opt = m_transactions.min();
        if( opt.hasValue()  ){
          (uint64 trId, Transaction txn) = opt.get() ;
          ...
          while (needCleanup && i < MAX_CLEANUP_TXNS) {
            ...
            opt = m_transactions.next(trId);
            needCleanup = opt.hasValue() ;
            if( needCleanup ) {
              (trId, txn) = opt.get() ;
    ...
    function getTransactions() public view returns (Transaction[] transactions) {
        ...
        optional(uint64, Transaction) opt = m_transactions.min();
        while (opt.hasValue()) {
          (uint64 id, Transaction txn) = opt.get() ;
            // returns only not expired transactions
            if (id > bound) {
                transactions.push(txn);
            }
            opt = m_transactions.next(id);
        }
    }
    ...
    function getCustodians() public view returns (CustodianInfo[] custodians) {
        optional(uint256, uint8) opt = m_custodians.min();
        while (opt.hasValue() ) {
          (uint256 key, uint8 index) = opt.get();
          custodians.push(CustodianInfo(index, key));
          opt = m_custodians.next(key);
        }
    }
    ...
    function confirmUpdate(uint64 updateId) public {
        uint8 index = _findCustodian(msg.pubkey());
        _removeExpiredUpdateRequests();
        optional(UpdateRequest) opt_request = m_updateRequests.fetch(updateId);
        require( opt_request.hasValue(), 115);
        UpdateRequest request = opt_request.get() ;
    ...
    function getUpdateRequests() public view returns (UpdateRequest[] updates) {
        uint64 bound = _getExpirationBound();
        optional(uint64, UpdateRequest) opt = m_updateRequests.min();
        while ( opt.hasValue() ) {
          (uint64 updateId, UpdateRequest req) = opt.get();
          if (updateId > bound) {
            updates.push(req);
          }
          opt = m_updateRequests.next(updateId);
        }
    }
