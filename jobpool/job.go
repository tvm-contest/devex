...
type Job struct {
	...
	RetryWait        int64         `json:"retrywait"`    // time which need to wait before retry (in UnixNano)
	RetryBackoff     string        `json:"retrybackoff"` // type of retry time calculation
	...
	After            int64         `json:"after"`        // time after which will perform (in UnixNano)
