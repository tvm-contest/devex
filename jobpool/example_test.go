...
func ExampleManager() {
	...
	m := jobpool.New(
		jobpool.SetConcurrency(0, 10),
		jobpool.SetConcurrency(1, 2),
	)

	// Register the processor for topic "crawl"
	jobDone := make(chan struct{}, 1)
	err := m.Register("crawl", func(job *jobpool.Job) error {
	...
	job := &jobpool.Job{Topic: "crawl", Args: []interface{}{"https://alt-f4.de"}}
